...
def pair_actor_loss(pair_actor_output, pair_selection, critic_loss_1, critic_loss_2, baseline_loss_1, baseline_loss_2, signal, lamda, margin):

    m = torch.nn.Softmax(dim=1)
    
    selection_1 = pair_selection[:, :46]
    selection_2 = pair_selection[:, 46:]
    
    actor_output_1 = pair_actor_output[:, :46]
    actor_output_2 = pair_actor_output[:, 46:]
...
def train_model(actor_model, critic_model, baseline_model, patience, epoch_start_early_stopping, saved_path, epochs, lamda, margin):
    ...
    for epoch in range(epochs):
        ...
        for batch, (data1, y1, data2, y2, signal) in enumerate(train_loader):
            ...
            pair_data = torch.cat((data1, data2), 1).to(device)
...
#             selection_2 = torch.bernoulli(actor_output_2)
            ...
            pair_actor_output = actor_model(pair_data)
            pair_selection = torch.bernoulli(pair_actor_output)
            ...
            actor_loss_output = pair_actor_loss(pair_actor_output, pair_selection, critic_loss_1, critic_loss_2, baseline_loss_1, baseline_loss_2, signal, lamda, margin)
        ...
        with torch.no_grad():   
            for batch, (data1, y1, data2, y2, signal) in enumerate(vali_loader):
                ...
                vali_pair_data = torch.cat((data1, data2), 1).to(device)
...
#                 vali_selection_2 = vali_actor_output_2.ge(0.5).type(torch.float)
                ...
                vali_pair_actor_output = actor_model(vali_pair_data)
                vali_pair_selection = torch.bernoulli(vali_pair_actor_output)

                vali_selection_1 = vali_pair_selection[:, :46]
                vali_selection_2 = vali_pair_selection[:, 46:]
                ...
                vali_actor_loss_output = pair_actor_loss(vali_pair_actor_output, vali_pair_selection, vali_critic_loss_1, vali_critic_loss_2, vali_baseline_loss_1, vali_baseline_loss_2, signal, lamda, margin)
